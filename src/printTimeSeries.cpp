...
int main(int argc, char * argv[]) {
  ...
  try {
    ...
    std::cerr << "\t -dada -dada_key ... -beams ... -batches ..." << std::endl;
    return 1;
  ...
  std::vector< uint8_t > zappedChannels(observation.getNrPaddedChannels(padding / sizeof(uint8_t)));
    ...
    if ( limit ) {
      AstroData::readLOFAR(headerFile, dataFile, observation, padding, *(input[0]), observation.getNrBatches());
    } else {
  ...
  observation.setNrDelayBatches(static_cast< unsigned int >(std::ceil(static_cast< double >(observation.getNrSamplesPerDispersedChannel()) / observation.getNrSamplesPerBatch())));
  ...
  // Dedispersion loop
  for ( unsigned int batch = 0; batch < observation.getNrBatches() - observation.getNrDelayBatches(); batch++ ) {
    for ( unsigned int beam = 0; beam < observation.getNrBeams(); beam++ ) {
      for ( unsigned int channel = 0; channel < observation.getNrChannels(); channel++ ) {
        for ( unsigned int chunk = 0; chunk < observation.getNrDelayBatches() - 1; chunk++ ) {
          if ( inputBits >= 8 ) {
            std::memcpy(reinterpret_cast< void * >(&(dispersedData[beam].data()[(channel * observation.getNrSamplesPerPaddedDispersedChannel(padding / sizeof(inputDataType))) + (chunk * observation.getNrSamplesPerBatch())])), reinterpret_cast< void * >(&((input[beam]->at(batch + chunk))->at(channel * observation.getNrSamplesPerPaddedBatch(padding / sizeof(inputDataType))))), observation.getNrSamplesPerBatch() * sizeof(inputDataType));
          } else {
            std::memcpy(reinterpret_cast< void * >(&(dispersedData[beam].data()[(channel * isa::utils::pad(observation.getNrSamplesPerDispersedChannel() / (8 / inputBits), padding / sizeof(inputDataType))) + (chunk * (observation.getNrSamplesPerBatch() / (8 / inputBits)))])), reinterpret_cast< void * >(&((input[beam]->at(batch + chunk))->at(channel * isa::utils::pad(observation.getNrSamplesPerBatch() / (8 / inputBits), padding / sizeof(inputDataType))))), (observation.getNrSamplesPerBatch() / (8 / inputBits)) * sizeof(inputDataType));
          }
        }
        if ( observation.getNrSamplesPerDispersedChannel() % observation.getNrSamplesPerBatch() == 0 ) {
          if ( inputBits >= 8 ) {
            std::memcpy(reinterpret_cast< void * >(&(dispersedData[beam].data()[(channel * observation.getNrSamplesPerPaddedDispersedChannel(padding / sizeof(inputDataType))) + ((observation.getNrDelayBatches() - 1) * observation.getNrSamplesPerBatch())])), reinterpret_cast< void * >(&((input[beam]->at(batch + (observation.getNrDelayBatches() - 1)))->at(channel * observation.getNrSamplesPerPaddedBatch(padding / sizeof(inputDataType))))), observation.getNrSamplesPerDispersedChannel() * sizeof(inputDataType));
          } else {
            std::memcpy(reinterpret_cast< void * >(&(dispersedData[beam].data()[(channel * isa::utils::pad(observation.getNrSamplesPerDispersedChannel() / (8 / inputBits), padding / sizeof(inputDataType))) + ((observation.getNrDelayBatches() - 1) * (observation.getNrSamplesPerBatch() / (8 / inputBits)))])), reinterpret_cast< void * >(&((input[beam]->at(batch + (observation.getNrDelayBatches() - 1)))->at(channel * isa::utils::pad(observation.getNrSamplesPerBatch() / (8 / inputBits), padding / sizeof(inputDataType))))), (observation.getNrSamplesPerDispersedChannel() / (8 / inputBits)) * sizeof(inputDataType));
          }
        } else {
          if ( inputBits >= 8 ) {
            std::memcpy(reinterpret_cast< void * >(&(dispersedData[beam].data()[(channel * observation.getNrSamplesPerPaddedDispersedChannel(padding / sizeof(inputDataType))) + ((observation.getNrDelayBatches() - 1) * observation.getNrSamplesPerBatch())])), reinterpret_cast< void * >(&((input[beam]->at(batch + (observation.getNrDelayBatches() - 1)))->at(channel * observation.getNrSamplesPerPaddedBatch(padding / sizeof(inputDataType))))), (observation.getNrSamplesPerDispersedChannel() % observation.getNrSamplesPerBatch()) * sizeof(inputDataType));
          } else {
            std::memcpy(reinterpret_cast< void * >(&(dispersedData[beam].data()[(channel * isa::utils::pad(observation.getNrSamplesPerDispersedChannel() / (8 / inputBits), padding / sizeof(inputDataType))) + ((observation.getNrDelayBatches() - 1) * (observation.getNrSamplesPerBatch() / (8 / inputBits)))])), reinterpret_cast< void * >(&((input[beam]->at(batch + (observation.getNrDelayBatches() - 1)))->at(channel * isa::utils::pad(observation.getNrSamplesPerBatch() / (8 / inputBits), padding / sizeof(inputDataType))))), ((observation.getNrSamplesPerDispersedChannel() % observation.getNrSamplesPerBatch()) / (8 / inputBits)) * sizeof(inputDataType));
          }
        }
      }
      ...
      for ( unsigned int sample = 0; sample < observation.getNrSamplesPerBatch() / outputIntegration; sample++ ) {
        output[beam] << static_cast< uint64_t >((batch * observation.getNrSamplesPerBatch()) + (sample / outputIntegration)) * (observation.getSamplingRate() * outputIntegration) << " " << integratedData[beam][sample] << std::endl;
      }
    }
  }
#ifdef HAVE_PSRDADA
