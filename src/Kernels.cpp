...
void generateSNROpenCLKernels(const OpenCLRunTime &openclRunTime, const AstroData::Observation &observation, const Options &options, const DeviceOptions &deviceOptions, const KernelConfigurations &kernelConfigurations, const HostMemory &hostMemory, const DeviceMemory &deviceMemory, Kernels &kernels)
{
    ...
    else if (options.snrMode == SNRMode::Momad)
    {
        ...
        if (options.subbandDedispersion)
        {
            code = SNR::getMedianOfMediansOpenCL<outputDataType>(*(kernelConfigurations.medianOfMediansStepTwoParameters.at(deviceOptions.deviceName)->at(observation.getNrDMs(true) * observation.getNrDMs())->at(observation.getNrSamplesPerBatch() / observation.getDownsampling() / options.medianStepSize)), SNR::DataOrdering::DMsSamples, outputDataName, observation, options.medianStepSize, observation.getDownsampling() * options.medianStepSize, deviceOptions.padding.at(deviceOptions.deviceName));
        }
        else
        {
